...
int f(int idx , int value_left){
    if(value_left==0) return 0;
    if(idx==n) return MAX;
    if(dp[idx][value_left]!=-1) return dp[idx][value_left];
    
    int val = f(idx+1,value_left);
    if(value_left >= arr[idx].second) val = min(val ,arr[idx].first + f(idx+1 , value_left-arr[idx].second));

    return dp[idx][value_left] = val;
}

inline void solve(){
    cin>>n>>w;
    arr.resize(n);
    vpin(arr);memset(dp,-1,sizeof(dp));
    // vpout(arr);
    for(int i=1e5;i>=0;i--){
        if(f(0,i) <= w) {cout<<i;break;}
    }
}
