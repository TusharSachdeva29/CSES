...
inline void solve(){
    ...
    dp[0]=1;
    // IN THIS ONE COIN CAN BE PICK MULTIPLE TIMES BUT IF U TRY TO REPLICAATE KIND OF SAME IN BOOK
    // SHOP THEN IT WOULD BE WRONG COZ ONE BOOK CAN OCCUR ONLY ONE TIME SO INSTEAD WRITING DIRECTLY OPTIMIZED WITH ONE VECTOR TRY TO USE TO PREV AND CURR FOR MORE CLEARITY
    for(itn j=0;j<n;j++){
        for(itn i=1;i<=k;i++){
            if(arr[j]<=i){
                dp[i]+=dp[i-arr[j]];
