...
struct Node1 {
    ll maxSum;    // Maximum subarray sum in the range
    ll totalSum;  // Total sum of the range
    ll prefixSum; // Maximum prefix sum
    ll suffixSum; // Maximum suffix sum
	Node1() {
        totalSum = 0;
        maxSum = 0;         // Handle empty subarray case
        prefixSum = 0;
        suffixSum = 0;
	}
	Node1(ll p1) {
        totalSum = p1;
        maxSum = max(0LL, p1); // Handle empty subarray case
        prefixSum = max(0LL, p1);
        suffixSum = max(0LL, p1);
	}
	void merge(Node1 &left, Node1 &right) {
...
// Store the following for each segment:
// maxSum: Maximum sum of any subarray in the segment.
// totalSum: Sum of all elements in the segment.
// prefixSum: Maximum sum of a prefix in the segment.
// suffixSum: Maximum sum of a suffix in the segment.

// Updation : 
//   1. parent.sum = l.sum + r.sum;
//   2. parent.prefix = max(l.prefix, l.sum + r.prefix);
//   3. parent.suffix = max(r.suffix, r.sum + l.suffix);
//   4. parent.maxSum = max({l.maxSum, r.maxSum, l.suffix + r.prefix}); 
...
void solve() {
	...
	// read(idx);read(k);
	cout<<seg.make_query(0,n-1).maxSum<<endl;
    // seg.make_update(idx,k);
    for(int i=0;i<m;i++){
        read(idx);read(k);
		seg.make_update(idx,k);
		cout<<seg.make_query(0,n-1).maxSum<<endl;
